...
import { IUserDonationScheduleReadPort } from '../../../../user/domain/ports/user-donation-schedule-read.port';
...
export class TriggerMonthlyDonationHandler implements IQueueHandler<TriggerMonthlyDonationJob> {
  ...
  constructor(
    @Inject(IUserDonationScheduleReadPort) private readonly scheduleReadPort: IUserDonationScheduleReadPort,
  ...
  async execute(job: Job<TriggerMonthlyDonationJob>, ctx: JobExecutionContext): Promise<void> {
    ...
    const users = await this.scheduleReadPort.findActiveDonors(userId);
