...
export class User extends AggregateRoot<string> {
   ...
   * Does not raise UserCreatedEvent — call {@link confirmProvisioned} once the
   * set-password ticket URL is known so welcome mail can include the link.
   */
  ...
  /**
   * Raises {@link UserCreatedEvent} after IdP link + optional set-password ticket.
   * Welcome correspondence uses title and setPasswordUrl from this event.
   */
  confirmProvisioned(setPasswordUrl?: string): void {
    if (!this.#idpSub) {
      throw new Error('confirmProvisioned requires linkIdentity() first');
    }
    this.addDomainEvent(
  ...
  /**
   * Projection sync for denormalized Auth role keys.
   * Does NOT emit UserProfileUpdatedEvent (no Auth0 / cache side-effects).
   */
  syncRoleKeys(keys: string[]): void {
    this.#roleKeys = [...new Set(keys.filter((k) => !!k?.trim()))];
  }
